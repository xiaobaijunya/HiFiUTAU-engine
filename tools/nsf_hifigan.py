...
from tools.utils import AttrDict, init_weights, get_padding
...
class SplitGenerator(torch.nn.Module):
    """拆分后的生成器，便于提取中间特征"""
    def __init__(self, h):
        ...
        for i, (u, k) in enumerate(zip(h.upsample_rates, h.upsample_kernel_sizes)):
            ch //= 2
            self.ups.append(weight_norm(nn.ConvTranspose1d(2 * ch, ch, k, u, padding=(k - u) // 2)))
            for j, (k, d) in enumerate(zip(h.resblock_kernel_sizes, h.resblock_dilation_sizes)):
                self.resblocks.append(resblock(h, ch, k, d))
            if not h.mini_nsf:
                if i + 1 < len(h.upsample_rates):
    ...
    def forward_part1(self, x):
        """第一部分：从输入到mini_nsf注入前"""
        x = self.conv_pre(x)
        for i in range(2):  # 只处理前两个上采样层
            x = F.leaky_relu(x, LRELU_SLOPE)
            x = self.ups[i](x)
            if i >= 1:
                continue
    ...
    def forward_part2(self, x, f0, nsf_gain=1, x_gain=1,out_har=False):
        """第二部分：从注入后到输出"""
        if self.mini_nsf:
            har_source = self.fastsinegen(f0)
            x_source = self.source_conv(har_source)*nsf_gain
            x = x*x_gain + x_source
        xs = None
        for j in range(self.num_kernels):
            if xs is None:
                xs = self.resblocks[1 * self.num_kernels + j](x)
            else:
                xs += self.resblocks[1 * self.num_kernels + j](x)
        x = xs / self.num_kernels
        
        # 继续处理剩余的上采样层
        for i in range(2, self.num_upsamples):
            x = F.leaky_relu(x, LRELU_SLOPE)
            x = self.ups[i](x)
            xs = None
            for j in range(self.num_kernels):
