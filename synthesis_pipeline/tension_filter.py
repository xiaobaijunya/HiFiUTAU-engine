...
# ═══════════════════════════════════════════════════════════════
#  3/4 次谐波增益（STFT 域频段提升）
# ═══════════════════════════════════════════════════════════════
def apply_harmonic_boost(waveform: np.ndarray,
                          hm34_map: np.ndarray,
                          sr: int = 44100,
                          hop_length: int = 512) -> np.ndarray:
    """在 STFT 域对 3、4 次谐波区域施加频段增益。

    3 次谐波区（~300-1500Hz）正常提升，
    4 次谐波区（~1500-4000Hz）减半提升。

    Args:
        waveform:  输入音频 (samples,)
        hm34_map:  逐样本 hm34 值 (0-100)
        sr:        采样率
        hop_length: STFT hop 长度

    Returns:
        处理后的音频 (samples,)
    """
    if len(waveform) == 0 or len(hm34_map) == 0:
        return waveform
    if np.max(np.abs(hm34_map)) < 0.5:
        return waveform
    ...
    hm34_frames = interp_to_len(hm34_map, n_frames)

    # 构建频域增益曲线
    fft_bin = n_fft // 2 + 1
    freqs = np.linspace(0, sr / 2, fft_bin)  # 每个频点的 Hz 值

    # 3 次谐波: 峰值约 800Hz, 范围 300-1500Hz, 峰比 1.0
    # 4 次谐波: 峰值约 2500Hz, 范围 1500-4000Hz, 峰比 0.5
    gain_curve = np.ones(fft_bin, dtype=np.float64)

    def _bell(f, center, spread, peak):
        """钟形曲线，spread 为半宽。"""
        return 1.0 + peak * np.exp(-0.5 * ((f - center) / (spread / 2.355)) ** 2)

    gain_curve *= _bell(freqs, 800, 700, 2.0)    # 3 次谐波 (提升更明显)
    gain_curve *= _bell(freqs, 2500, 1500, 1.0)   # 4 次谐波 (减半, 相对 3 次)
    gain_curve = np.maximum(gain_curve, 1.0)       # 基准 >1

    # 逐帧应用
    mag_db = np.log(np.clip(mag, 1e-12, None))
    for t in range(n_frames):
        hm = hm34_frames[t] / 100.0  # -100~100 → -1.0~1.0
        if abs(hm) < 0.005:
            continue
        # 帧能量保持
        orig_sum = np.sum(mag[:, t])
        mag_db[:, t] += np.log(gain_curve) * hm
        if orig_sum > 1e-12:
            new_sum = np.sum(np.exp(mag_db[:, t]))
            if new_sum > 1e-12:
                mag_db[:, t] += np.log(orig_sum / new_sum)
